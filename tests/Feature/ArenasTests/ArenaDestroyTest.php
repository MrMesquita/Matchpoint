...
describe('destroy an arena', function () {
    test("admin can be destroy an arena", function () {
        $response = $this->actingAs($this->adminUser)->deleteJson(route('arenas.destroy', ['arena' => $this->arena->id]));
    ...
    test("system can be destroy an arena", function () {
        ...
        $response = $this->actingAs($this->systemUser)->deleteJson(route('arenas.destroy', ['arena' => $newArena->id]));
    ...
    test('try to destroy an arena without logged in', function() {
        $response = $this->deleteJson(route('arenas.destroy', ['arena' => $this->arena->id]));
    ...
    test("admin try to destroy an arena that doesn't exist or that doesn't belong to him", function() {
        $response = $this->actingAs($this->adminUser)->deleteJson(route('arenas.destroy', ['arena' => 0]));
